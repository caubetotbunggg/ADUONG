...
def teacher_pseudo_labels(
    ...
    images: torch.Tensor,
    conf_thresh: ThreshType = 0.7,
    aat_generator: Optional[object] = None,
) -> List[Dict[str, torch.Tensor]]:
    """Pseudo-labels from a teacher; uses AAT (clean+attacked merge) when provided."""
    if aat_generator is not None:
        merged, _ = aat_generator.generate(teacher, images, conf_thresh)
        return merged
    with torch.no_grad():
...
def compute_rgb_mid_loss(
    ...
    aat_generator: Optional[object] = None,                 # AAT pseudo-label generator
) -> Tuple[torch.Tensor, Dict[str, float]]:
    """
    ...
    if config.p2_rgb_teacher_weight > 0.0:
        pseudo = teacher_pseudo_labels(rgb_teacher, t_images, conf_thresh, aat_generator)
    ...
    if config.p2_ir_teacher_weight > 0.0:
        pseudo = teacher_pseudo_labels(ir_teacher, t_images, conf_thresh, aat_generator)
